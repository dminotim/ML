...
std::shared_ptr<layers::mwLayer<Scalar>> AddConvolution(mwCNN<Scalar>& cnn, const size_t fc,
	const size_t kernel,
	const mwTensorView<Scalar>& inShape, bool uselastRelu = true, bool useAddConv = true)
{
	/*auto zeroP = std::make_shared<layers::mwZeroPaddingLayer<Scalar>>(1, inShape);
	cnn.AddLayer(zeroP);*/
	auto convolv = std::make_shared<layers::mwConv2dLayer<Scalar>>(fc, kernel,
		layers::mwPaddingType::SAME, inShape);
	...
	if (uselastRelu && useAddConv)
	{
		convolv = std::make_shared<layers::mwConv2dLayer<Scalar>>(fc, kernel,
			layers::mwPaddingType::SAME, cnn.Layers().back()->GetOutShape());
		cnn.AddLayer(convolv);
	}
	if (uselastRelu)
	{
	...
	const mwTensorView<Scalar>& inShape)
{
	...
	//cnn.AddLayer(zeroP);
	auto convolv = std::make_shared<layers::mwConv2dLayer<Scalar>>(fc, 3,
		layers::mwPaddingType::SAME, up1->GetOutShape());
	cnn.AddLayer(convolv);
	auto relu = std::make_shared<layers::mwReluLayer<Scalar>>(convolv->GetOutShape());
	cnn.AddLayer(relu);
	return convolv;
...
{
	auto conv1 = AddConvolution(cnn, 32, 3, inputShape); size_t c1Idx = cnn.Layers().size() - 1;
	auto pool1 = AddPooling(cnn, conv1->GetOutShape());
	auto conv2 = AddConvolution(cnn, 64, 3, pool1->GetOutShape()); size_t c2Idx = cnn.Layers().size() - 1;
	auto pool2 = AddPooling(cnn, conv2->GetOutShape());
	auto conv3 = AddConvolution(cnn, 128 , 3, pool2->GetOutShape()); size_t c3Idx = cnn.Layers().size() - 1;
	auto pool3 = AddPooling(cnn, conv3->GetOutShape());
	auto conv4 = AddConvolution(cnn, 256 , 3, pool3->GetOutShape()); size_t c4Idx = cnn.Layers().size() - 1;
	...
	auto conv5 = AddConvolution(cnn, 512 , 3, pool4->GetOutShape()); size_t c5Idx = cnn.Layers().size() - 1;
	...
	auto conv6 = AddConvolution(cnn, 256, 3, merge1->GetOutShape());
	auto up2 = AddUpsampling(cnn, 128 , conv6->GetOutShape());
	cnn.AddLayer(std::make_shared<layers::mwConcatLayer<Scalar>>(
		cnn.Layers(), std::vector<size_t>(1, c3Idx), up2->GetOutShape()));
	auto conv7 = AddConvolution(cnn, 128, 3, cnn.Layers().back()->GetOutShape());
	auto up3 = AddUpsampling(cnn, 64 , conv7->GetOutShape());
	cnn.AddLayer(std::make_shared<layers::mwConcatLayer<Scalar>>(
		cnn.Layers(), std::vector<size_t>(1, c2Idx), up3->GetOutShape()));
	auto conv8 = AddConvolution(cnn, 64, 3, cnn.Layers().back()->GetOutShape());
	auto up4 = AddUpsampling(cnn, 32, conv8->GetOutShape());
	cnn.AddLayer(std::make_shared<layers::mwConcatLayer<Scalar>>(
		cnn.Layers(), std::vector<size_t>(1, c1Idx), up4->GetOutShape()));

	auto conv10 = AddConvolution(cnn, 32 , 3, cnn.Layers().back()->GetOutShape());
	auto conv11 = AddConvolution(cnn, 2, 3, cnn.Layers().back()->GetOutShape(), true, false);
	auto conv12 = AddConvolution(cnn, 1, 1, cnn.Layers().back()->GetOutShape(), false);
